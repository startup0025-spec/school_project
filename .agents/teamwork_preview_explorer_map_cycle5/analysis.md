# Kakao Map Integration: Final Code Construction (Cycle 5)

This document provides the Final Code Construction and architectural specifications for the Kakao Map API Integration in *Anyway, the Sea*. All proposed changes have been verified against the project's TypeScript compiler environment and run cleanly.

---

## 1. Synthesis of Architectural Insights & Mitigations

This design synthesizes crucial performance, layout, and stability mitigations developed across all cycles:

1. **Grayscale CSS Filter (Calm UX)**: To integrate Kakao Map tiles seamlessly into the app's dark indigo aesthetic without distorting custom SVGs or brand elements, we target tile images via specific hostname paths in CSS:
   ```css
   ...
   ```
2. **Base URL Spoofing**: Loads the inline HTML string into the WebView while spoofing the origin domain to `https://haetae05.github.io` to bypass Kakao JS SDK web domain validation restrictions.
3. **Location Proxying**: Native GPS tracking runs via Expo's `Location.watchPositionAsync` in React Native and coordinates are proxied into the WebView using `injectJavaScript` to bypass webview permission blocks.
4. **WebView Pointer-Event Toggling**: Disables pointer-events when the screen is blurred (`pointerEvents={isFocused ? 'auto' : 'none'}`) to prevent background WebView resize recalculations when the keyboard is displayed on other screens.
5. **Full Dimensions Keep-Alive (WebGL Crash Avoidance)**: Instead of collapsing the container style (`width: 0, height: 0` or `display: 'none'`) during tab blurs—which causes WebKit/Chromium context discards and canvas rendering crashes—we keep the viewport active at full dimensions but transition it off-screen:
   ```typescript
   ...
   ```
6. **WebGL Context Loss Handler**: Embeds WebGL/canvas recovery event listeners in the HTML and native bridge to reload or relayout the viewport upon focus, ensuring a non-broken visual state.
7. **Custom Cache Subscription Listener**: Resolves the SWR cache update race condition by adding a subscription listener to `local_places.ts` that triggers reactive UI updates in `map.tsx` immediately when background revalidations finish writing fresh CDN data to AsyncStorage.
8. **Calibrated Walking Time**: Resolves straight-line distance underestimations by applying an urban routing multiplier (1.35x) and Busan-hill adjusted walking speed (65 meters/minute), with NaN-guarding fallbacks.

---

## 2. Final Copy-Pasteable Code Blocks

### 2.1 File: `mobile/constants/mockData.ts`
This cleaned-up mock data structure removes the relative pixel `pin` properties, adds real geographic Busan water station coordinates, and refactors `QuietSpot` to extend the core `Place` model.
...
### 2.2 File: `mobile/core_engine/src/database/local_places.ts`
Adds a Set-based Cache Subscription Listener mechanism allowing components to reactive-subscribe to AsyncStorage cache overrides triggered by SWR background fetches.
...
### 2.3 File: `mobile/app/(tabs)/map.tsx`
The complete production-ready file for the tab screen. Features all performance preservation, gesture management, cache sync, error boundary handling, and aesthetic custom tiles.
