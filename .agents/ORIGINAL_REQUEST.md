...
## Follow-up — 2026-07-23T23:18:37+09:00

Conduct a deep, exhaustive, and independent code audit and quality assurance sweep of the entire 'Anyway_the_Sea' React Native project. Identify any hidden bugs, React rendering flaws, type errors, memory leaks, or architectural weaknesses across the entire codebase. Take as much time as needed to be absolutely thorough.
...
Review the core logic in `mobile/app/(tabs)`, `mobile/lib/services`, and the data pipeline scripts (`scripts/pipeline`). Pay special attention to the Kakao Map WebView bridging and the DSP audio mixing logic.
Note: The data pipeline (`bake_places.js`) does NOT use OpenAI (it uses a local fallback), so do not flag missing OpenAI logic as an error.

### R2. Bug Detection & Reporting
Identify potential runtime errors, React hook dependency issues, memory leaks (especially missing cleanup in `useEffect` or `setInterval`), and unhandled edge cases. Do not introduce new features; focus strictly on auditing and fixing.

## Acceptance Criteria

### Audit Verification
- [ ] A programmatic type check (`npx tsc --noEmit` inside `mobile/`) must be executed to verify basic type safety.
- [ ] A final `audit_report.md` must be generated detailing all found vulnerabilities, bugs, or architectural weaknesses, and ideally, any code fixes should be applied and verified.
...
## Follow-up — 2026-07-24T13:29:37+09:00
...
## Follow-up — 2026-07-24T13:33:06+09:00

[HIGH PRIORITY USER ADDENDUM]: The User is furious and explicitly commands that you must audit EVERYTHING. This includes the BACKEND (e.g., `scripts/pipeline/bake_places.js`, server-side logic, GitHub Actions, data baking processes) as well as the FRONTEND. You must trace the entire architecture from the backend scrapers/pipelines all the way to the frontend UI. Do NOT skip the backend. Expand your forensic audit scope immediately.
